...
const ImagesSlide: React.FC<Props> = ({ images: { backdrops } }) => {
    // console.log("the image: ", backdrops);
        ...
        <div className="ImagesSlide mb-4">
            <Slider {...settings}>
                {backdrops.map((image: any, index: number) => (
                    <MovieImages image={image} key={index} openOverlay={openOverlay} length={backdrops.length} />
                ))}
            </Slider>
            {showOverlay ? (
                <>
                    <div className="overlay" onClick={overlayClickHandler} ref={overlayRef}>
                        <button onClick={() => setShowOverlay(false)} type="button" className="close" aria-label="Close">
